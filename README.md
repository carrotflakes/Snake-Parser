...
文法ファイルは文法を記述したテキストファイルです。出力パスにJavaScriptコードが生成されます。
出力パスを省略すれば、文法ファイルの拡張子を `.js` にしたものが出力パスに設定されます。
また文法ファイルと出力パスを両方省略すれば標準入力から文法を受け取って標準出力からコードを出力するようになります。
出力コードの内容は、変数 `module.export` へのパーサ関数の代入になります。`snakeparser` コマンドのオプション `--export-var` (または `-e`) を使うと代入する変数名を変更できます。
...
##### `\V`
`V` は Number, String, Boolean, null, Array, Object で構成されるJavaScriptの値です。`V` がそのまま返り値になります。  
e.g. `\123` `\"Hello"` `\true` `\null` `\[1,2,3]` `\{a: 1, b: 2}`
